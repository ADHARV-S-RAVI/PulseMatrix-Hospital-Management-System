...
export default function PatientDashboard({ patientId, onLogout }) {
    ...
    async function fetchPatientData() {
      try {
        ...
        setPatient({
          ...
          severity: severity,
          status: "In Treatment", // Backend doesn't have status, so default to In Treatment
          assignedDoctor: null,   // Backend doesn't have assignedDoctor
          assignedBed: null,      // Backend doesn't have assignedBed
          admittedTime: new Date(data.admission_date).toLocaleString(),
          contact: "555-0199"     // Backend doesn't store contact
        });
      } catch (err) {
        console.error("Error fetching patient", err);
        setError("Your case record could not be loaded.");
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="d-flex align-items-center justify-content-center" style={{ minHeight: "60vh" }}>
        <div className="text-center">
          ...
          <h3 className="fw-bold">Loading Case Data...</h3>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="d-flex align-items-center justify-content-center" style={{ minHeight: "60vh" }}>
        <div className="text-center">
          ...
          <p className="text-muted">{error || "Your case record could not be loaded."}</p>
  ...
  const assignedDoctor = null; // Removed mock doctor link
  const assignedBed    = null; // Removed mock bed link
